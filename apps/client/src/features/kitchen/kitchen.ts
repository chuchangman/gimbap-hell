/* ────────────────────────────────────────────────────────────
   ...
   여기서는 서버 스냅샷(S.kitchen)을 보고
     ...
     · 3D 가 그릴 진행도를 계산한다
   실제 판정은 서버(apps/server 의 Kitchen)가 다시 한다.

   레거시 public/js/kitchen.js 를 그대로 옮겼다.
   ──────────────────────────────────────────────────────────── */
import { act, emit, isPlaying, myHand, S, serverNow } from '@/features/net/net';
import {
  BURNERS,
  cookStatus,
  focusPick,
...
/** 조준한 설비 — hitProxy 가 userData.station 에 넣어 둔 것.
 *  판별 유니온이라 switch (st.kind) 안에서 필요한 필드가 반드시 있다 —
 *  레거시는 전부 optional 이라 오타가 조용히 undefined 로 흘렀다. */
export type StationRef =
  | { kind: 'fridge'; item: ItemId }
  | { kind: 'sink' }
  | { kind: 'cooker'; cooker: number }
  | { kind: 'burner'; slot: number }
  | { kind: 'board'; board: number }
  | { kind: 'mat'; mat: number }
  | { kind: 'bin' }
  | { kind: 'broom'; rack: number }
  | { kind: 'serve' }
...
const sec = (at: number): number => (serverNow() - at) / 1000;
const K = () => S.kitchen;

/* ──────────────── 진행도 조회 (world.js 용) ──────────────── */
export function sinkAt() {
  const k = K();
  return k ? k.sink : null;
}

export function cookerAt(i: number) {
  const k = K();
...
export function burnerInfo(slot: number) {
  const k = K();
  const cell = k?.burners[slot];
  if (!cell) return null;
...
export function boardInfo(i: number) {
  const k = K();
  const b = k?.boards[i];
  if (!b) return null;
  const el = sec(b.at);
...
export function activeMat() {
  ...
  let best = m,
    n = m ? m.fills.length : -1;
  k.mats.forEach((x, i: number) => {
    if (x.fills.length > n) {
      best = x;
      n = x.fills.length;
      lastMat = i;
...
function matAct(action: string, i: number): void {
  lastMat = i;
  act(action, { mat: i });
}

export function matAt(i: number) {
  const k = K();
...
export function missingFills(i: number): ItemId[] {
  const m = matAt(i);
  const w = S.state && S.state.wave;
...
export function targetMatch(target: { fills: ItemId[] } | null | undefined): number | null {
  const h = myHand();
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
export function resolveAction(st: StationRef | null | undefined): ActionHint | null {
  if (!st) return null;
  const k = K();
  if (!k) return null;
  const h = myHand();
  ...
  if (st.kind === 'broom') {
    ...
    return {
      ...
      key: 'E',
      danger: true,
      run: () => act('broom:take', { rack: st.rack }),
    };
  }
  ...
  switch (st.kind) {
    case 'fridge': {
      ...
      return {
        ...
        key: 'E',
        run: () => act('fridge:take', { item: st.item }),
      };
    }
    ...
    case 'cooker': {
      ...
      if (c.state === 'cooking') {
        ...
        return {
          text: '밥솥 ' + (i + 1) + ' 취사 중... ' + left.toFixed(1) + '초',
          disabled: true,
        };
      }
      if (c.state === 'ready') {
        ...
        return {
          text: '밥 푸기 — ' + c.servings + '인분 남음',
          key: 'E',
          run: () => act('cooker:take', { cooker: i }),
        };
      }
      if (h && h.id === 'rice' && h.stage === 'washed') {
        return {
          text: '쌀 안치기 — ' + TIME.riceCook + '초 취사',
          key: 'E',
          run: () => act('cooker:put', { cooker: i }),
        };
      }
    ...
    case 'burner': {
      ...
      if (info) {
        if (h)
          return {
            text: ITEMS[info.cell.id].name + ' — ' + info.label + ' (손이 참)',
            disabled: true,
          };
        return {
          ...
          key: 'E',
          danger: info.burnt,
          run: () => act('burner:take', { slot: st.slot }),
        };
      }
      if (h && h.stage === 'raw' && ITEMS[h.id] && ITEMS[h.id].station === b.kind) {
        ...
        return {
          ...
          key: 'E',
          run: () => act('burner:put', { slot: st.slot }),
        };
      }
      const want = (Object.keys(ITEMS) as ItemId[])
        .filter((id) => ITEMS[id].station === b.kind && unlockedFills().includes(id))
        .map((id) => ITEMS[id].emoji + ITEMS[id].name)
        .join(' · ');
    ...
    case 'board': {
      ...
      if (h && ITEMS[h.id] && ITEMS[h.id].station === 'board' && h.stage === 'raw') {
        return {
          ...
          key: 'E',
          run: () => act('board:put', { board: st.board }),
        };
      }
      if (h && h.id === 'roll') {
        return {
          text: '김밥 썰기 — ' + TIME.cutRoll + '초',
          key: 'E',
          run: () => act('board:put', { board: st.board }),
        };
      }
      return {
        text: '도마 ' + (st.board + 1) + ' — 단무지·오이나 만 김밥을 들고 오세요',
        disabled: true,
      };
    }
    ...
    case 'mat': {
      ...
      if (h) {
        ...
        if (ITEMS[h.id] && ITEMS[h.id].fill) {
          if (h.stage === 'raw')
            return {
              text: '손질 안 된 ' + ITEMS[h.id].name + ' 은(는) 못 넣습니다',
              disabled: true,
            };
          if (h.stage === 'burnt')
            return { text: '못 쓰게 된 재료입니다 (음쓰통으로)', disabled: true };
          if (!m.bap) return { text: '김 → 밥 순서로 먼저 올리세요', disabled: true };
          if (m.fills.some((f) => f.id === h.id))
            return { text: ITEMS[h.id].name + ' 은(는) 이미 들어갔습니다', disabled: true };
      ...
      if (m.gim && m.bap && m.fills.length) {
        ...
        return {
          text:
            ...
            '종' +
            (miss.length
              ? ' · ' + miss.map((f) => ITEMS[f].name).join('·') + ' 빠짐'
              : ' · 주문 충족') +
            ')',
          key: 'E',
          danger: miss.length > 0,
          run: () => matAct('mat:roll', i),
        };
      }
      if (m.gim || m.bap || m.fills.length) {
    ...
    case 'customer': {
      ...
      if (h && h.id === 'roll')
        return { text: who + ' — 먼저 도마에서 썰어 오세요', disabled: true };
    ...
    case 'serve': {
      ...
      if (h && h.id === 'gimbap') {
        ...
        return {
          text:
            who + ' 에게 서빙! (' + (t.done + 1) + '/' + t.need + '줄 · 주문 일치 ' + pct + '%)',
          key: 'E',
          danger: pct < 100,
          run: () => act('serve'),
        };
...
/** 좌클릭 스윙 */
export function swingBroom(targetId?: string | null, targetKind?: string | null): void {
